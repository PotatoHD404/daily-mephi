...
export const searchRouter = t.router({
    search: t.procedure.meta({
        ...
        .output(z.any())
        .query(async ({ctx: {prisma}, input: {query, sort, facultyIds, disciplineIds, ratingFrom, ratingTo, limit, offset, types}}) => {
        ...
        WITH ${mustNotBe.length > 0 ? Prisma.sql`t as (SELECT ARRAY [UNNEST(CAST(${mustNotBe} AS TEXT[]))] as word),` : Prisma.empty}
             t1 as (SELECT *
                    FROM "Document" ${mustBe.length > 0 || types ? Prisma.sql`WHERE` : Prisma.empty}
